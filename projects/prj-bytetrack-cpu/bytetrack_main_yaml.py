...
async def main():
    ...
    bytetrack_config = config_loader.get_service_config('bytetrack')
    
    # Get configuration values (CLI args override YAML)
    in_mqtt = args.in_mqtt if args.in_mqtt else config_loader.get_input_config_string('bytetrack')
    out_mqtt = args.out_mqtt if args.out_mqtt else config_loader.get_output_config_string('bytetrack')
    devices = args.devices if args.devices else config_loader.get_devices('bytetrack')
    ...
    try:
        ...
        model_config = dict(
            ...
            frame_rate=bytetrack_config.get('frame_rate', 30),
